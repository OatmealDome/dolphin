...
BytecodeInterpreter::BytecodeInterpreter(Core::System& system) : JitBase(system), m_block_cache(*this)
{
}
...
void BytecodeInterpreter::Shutdown()
{
  m_block_cache.Shutdown();
}

void BytecodeInterpreter::ExecuteOneBlock()
{
...
void BytecodeInterpreter::SingleStep()
{
...
bool BytecodeInterpreter::HandleFunctionHooking(u32 address)
{
  // BytecodeInterpreter inherits from JitBase and is considered a JIT by relevant code.
...
void BytecodeInterpreter::WriteEndBlock()
{
...
bool BytecodeInterpreter::SetEmitterStateToFreeCodeRegion()
{
...
void BytecodeInterpreter::FreeRanges()
{
...
void BytecodeInterpreter::ResetFreeMemoryRanges()
{
  m_free_ranges.clear();
...
void BytecodeInterpreter::Jit(u32 em_address, bool clear_cache_and_retry_on_failure)
{
...
bool BytecodeInterpreter::DoJit(u32 em_address, JitBlock* b, u32 nextPC)
{
...
void BytecodeInterpreter::EraseSingleBlock(const JitBlock& block)
{
  m_block_cache.EraseSingleBlock(block);
  FreeRanges();
}

std::vector<JitBase::MemoryStats> BytecodeInterpreter::GetMemoryStats() const
{
  return {{"free", m_free_ranges.get_stats()}};
}

std::size_t BytecodeInterpreter::DisassembleNearCode(const JitBlock& block,
                                                   std::ostream& stream) const
{
  return Disassemble(block, stream);
}

std::size_t BytecodeInterpreter::DisassembleFarCode(const JitBlock& block, std::ostream& stream) const
{
  stream << "N/A\n";
  return 0;
}

void BytecodeInterpreter::ClearCache()
{
  m_block_cache.Clear();
  m_block_cache.ClearRangesToFree();
  ClearCodeSpace();
...
void BytecodeInterpreter::LogGeneratedCode() const
{
